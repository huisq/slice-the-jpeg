...
module overmind::slice_the_jpeg {
    ...
    use aptos_token_objects::collection::Collection;        // https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-framework/doc/event.md#module-0x1event
    use std::bcs;
    //use std::debug;
    ...
    fun init_module(account: &signer) {
        ...
        let collection_address = create_split_collection(&resource_signer);
    ...
    public entry fun split<NFT: key >(
        ...
        check_if_account_owns_nft<NFT>(signer::address_of(nf_token_owner_signer), nf_token_object);
        ...
        let creator_address = token::creator(nf_token_object);
        let state = borrow_global_mut<State>(@overmind);
        let res_signer = account::create_signer_with_capability(&state.signer_capability);
        let split_collection_obj = object::address_to_object<Collection>(state.collection_address);
        let split_collection_name = collection::name(split_collection_obj);
        let new_split_token_address = token::create_token_address(&signer::address_of(&res_signer), &split_collection_name, &split_token_name);
        check_if_split_token_does_not_exist(new_split_token_address);

        // TODO: Transfer the NFT to the module's resource account
        object::transfer(nf_token_owner_signer,nf_token_object,signer::address_of(&res_signer));
        ...
        let nft_token_owner_address = signer::address_of(nf_token_owner_signer);
        mint_internal(object::address_to_object<SplitToken>(new_split_token_address), nft_token_owner_address, split_token_supply);

        // TODO: Emit a new SplitEvent 
        event::emit_event<SplitEvent>(
            &mut state.split_events,
            SplitEvent{
                splitter_address: nft_token_owner_address,
                nft_address: nf_token_address,
                split_token_address: new_split_token_address,
                split_token_supply,
                split_token_call_threshold,
                split_token_call_price
            });
    ...
    public entry fun redeem<NFT: key>(
        ...
        check_if_nf_token_is_owned_by_account<NFT>(nft_address,signer::address_of(&res_signer));
        ...
        check_if_split_token_balance_is_equal_to_supply(
            split_balance(coin_owner_address, split_token_object),
            split_token_supply
        );
        ...
        burn_internal(coin_owner, split_token_object, split_token_supply);
        ...
        object::transfer(&res_signer,nft_object,coin_owner_address);
    ...
    public entry fun call<NFT: key>(
        ...
        let split_token_threshold = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_CALL_THRESHOLD));
        check_if_split_token_balance_is_equal_to_or_greater_than_call_threshold(
            split_token_balance,
            split_token_threshold
        );

        // Missed To-do: If coin owner's split token balance is equal split token supply
        let split_token_supply = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_MAX_SUPPLY));
        let aptos_coin_balance = coin::balance<AptosCoin>(coin_owner_address);
        let call_payment = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_CALL_PRICE));
        if(split_token_balance != split_token_supply){
            ...
            //       resource account
            coin::transfer<AptosCoin>(coin_owner, signer::address_of(&res_signer), (split_token_supply-split_token_balance)*call_payment);
        };
    ...
    public entry fun exchange_split_tokens_for_call_payment<NFT: key >(
        ...
        let res_signer = account::create_signer_with_capability(&state.signer_capability);
        let nft_address = property_map::read_address<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_NFT_ADDRESS));
        check_if_nf_token_is_not_owned_by_account<NFT>(nft_address, signer::address_of(&res_signer));
        ...
        //       owner
        let call_payment = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_CALL_PRICE));
        let aptos_amount = call_payment * split_token_balance;
        coin::transfer<AptosCoin>(&res_signer, coin_owner_address, aptos_amount);
        ...
        event::emit_event<ExchangeEvent>(
            ...
            ExchangeEvent{
                ...
                aptos_coin_exchange_payment: aptos_amount
            });
    }
    ...
    fun create_split_token_as_fungible_token(
        ...
        let token_const_ref = token::create_named_token(creator,name,description,fungible_asset_name,option::none(),uri);
        ...
        let prop_values = vector[
            bcs::to_bytes(&call_price),
            bcs::to_bytes(&call_threshold),
            bcs::to_bytes(&max_supply),
            bcs::to_bytes(&nft_address),
            bcs::to_bytes(&nft_collection_name),
            bcs::to_bytes(&nft_creator_address),
            bcs::to_bytes(&nft_token_name)
        ];
        ...
        //       token object
        let new_split_token = SplitToken{
            property_mutator_ref: property_map::generate_mutator_ref(&token_const_ref),
            fungible_asset_mint_ref: fungible_asset::generate_mint_ref(&token_const_ref),
            fungible_asset_burn_ref: fungible_asset::generate_burn_ref(&token_const_ref)
        };
        move_to(&obj_signer,new_split_token);
    }
    ...
    public fun split_balance(account: address, split_token: Object<SplitToken>): u64 {
        ...
        let account_store = primary_fungible_store::ensure_primary_store_exists(account, split_token);
        fungible_asset::balance(account_store)
    }
    ...
    fun mint_internal(split_token_object: Object<SplitToken>, receiver: address, amount: u64) acquires SplitToken {
        ...
        let recipient_store = primary_fungible_store::ensure_primary_store_exists(receiver, split_token_object);
        fungible_asset::mint_to(&split_token.fungible_asset_mint_ref, recipient_store,amount);
    }
    ...
    fun burn_internal(owner: &signer, split_token_object: Object<SplitToken>, amount: u64) acquires SplitToken {
        ...
        // HINT: https://aptos.dev/standards/aptos-token-v2/#fungible-token
        let split_token = borrow_global<SplitToken>(object::object_address(&split_token_object));
        let obj_store = primary_fungible_store::ensure_primary_store_exists(signer::address_of((owner)), split_token_object);
        fungible_asset::burn_from(&split_token.fungible_asset_burn_ref, obj_store, amount);
    }
    ...
    inline fun check_if_nf_token_is_owned_by_account<NFT: key>(nf_token_address: address, account_address: address) {
       assert!(object::owner(object::address_to_object<NFT>(nf_token_address)) == account_address, ENftIsNotOwnedByAccount);
    }
    ...
    inline fun check_if_nf_token_is_not_owned_by_account<NFT: key>(nf_token_address: address, account_address: address) {
        assert!(object::owner(object::address_to_object<NFT>(nf_token_address)) != account_address, ENftIsOwnedByAccount);
    }
